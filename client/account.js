window.addEventListener("load", async function() {
    const loggedin = await fetch('/account');
        ...
        document.getElementById("change-pwd").addEventListener('click', async function () {
            if(document.getElementById('password').readOnly === true){
                ...
                document.getElementById("change-pwd").addEventListener('click', async function () {
                    ...
                    try {
                        await fetch(`/user/password?user_id=${user_id}&new=${newPwd}`);
        ...
        if (!response.ok) {
            console.error("Could not view rides.");
                ...
                if(rides.active !== undefined && rides.active.length > 0) {
                    ...
                    for(const ride of rides.active) {
...
function buildRides(table,tb,ride, pending) {
    ...
    if(!pending){
        td = document.createElement('td');
        const message = "Hey! Where should we meet?";
...
async function accept() {
    ...
    const to = JSON.stringify(JSON.parse(window.localStorage.getItem('me'))._id);
...
async function cancel(){
    ...
    const response = await fetch('/user/rides/delete?user_id=' + user_id + '&ride_id=' + ride_id, {credentials: 'include'});
    if(!response.ok) {
